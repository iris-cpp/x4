...
int main()
{
    ...
    BOOST_SPIRIT_X3_ASSERT_CONSTEXPR_CTORS(attr);
...

    {
        ...
        std::pair<int, int> p;
        BOOST_TEST(parse("1", int_ >> attr(1), p) &&
        ...
        str.clear();
        BOOST_TEST(parse("", attr(std::string("test")), str))
          && BOOST_TEST_EQ(str, "test");

        int array[] = {0, 1, 2};
        std::vector<int> vec;
        BOOST_TEST(parse("", attr(array), vec) && vec.size() == 3 &&
            vec[0] == 0 && vec[1] == 1 && vec[2] == 2);
    }

    {
        std::string s;
        BOOST_TEST(parse("s", "s" >> attr(std::string("123")), s) &&
            s == "123");
