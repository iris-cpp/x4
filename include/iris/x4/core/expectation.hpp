...
template<std::forward_iterator It, class Subject, class Context>
constexpr void set_expectation_failure_if_empty(
    It where,
    Subject const& subject,
    Context const& ctx
)
{
    ...
    auto& failure = x4::get<contexts::expectation_failure>(ctx);
    if (!failure) failure.emplace(std::move(where), x4::what(subject));
}
